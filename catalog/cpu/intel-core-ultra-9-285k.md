...
specs:
  ...
  lithography: "TSMC N3B (Compute tile) + TSMC N6 (SoC tile) + TSMC N6 (GPU tile)"
  cores: 24
  threads: 24
  p_cores: 8
  e_cores: 16
  ...
  tdp_pl2: "250W"
  tjmax: "105°C"
  pcie_lanes: "20 CPU (16× PCIe 5.0 + 4× PCIe 4.0), доп. от чипсета Z890"
  pcie_version: "5.0"
...
verdict: "Флагман Intel на LGA1851 с радикально улучшенной энергоэффективностью. 125W против 253W у предшественника (i9-14900K). В многопотоке и productivity — силён (QuickSync + NPU). В играх проигрывает AMD Ryzen X3D, но для рабочих станций с видеомонтажом — один из лучших выборов."
---
...
**Ключевое достижение:** 285K при TDP 125W (PL2 250W) сопоставим по многопоточной производительности с i9-14900K, который потреблял 253W (PL2 280W+). Флагман Intel теперь потребляет **вдвое меньше** в пике.

Процессор ориентирован на создателей контента, видеомонтажёров и профессионалов. QuickSync — по-прежнему лучший аппаратный энкодер на рынке для Premiere Pro и DaVinci Resolve. Добавлен NPU (13 TOPS) для AI-ускорения. В играх — честно уступает Ryzen 7 7800X3D / 9800X3D, но не критично для 4K-гейминга.
...
- **SoC tile (TSMC N6):** контроллер памяти DDR5, PCIe 5.0, NPU.
- **GPU tile (TSMC N6):** Intel Graphics на архитектуре Xe-LPG (4 Xe-ядра). QuickSync, аппаратное декодирование AV1/HEVC/VP9.
- **I/O tile:** дополнительные линии PCIe 4.0, USB, SATA через чипсет.
...
- E-cores Skymont: база 3.2 GHz, boost до 4.6 GHz
...
- TJmax: 105°C
- iGPU: Intel Graphics (4 Xe-LPG, до 2.0 GHz) — QuickSync, 4× дисплея, HDMI 2.1, DP 2.1
- NPU: Intel AI Boost (13 TOPS) — для AI-нагрузок
...
- Максимальный объём: 192 GB (4×48 GB)
- PCIe от CPU: 20 линий (16× PCIe 5.0 + 4× PCIe 4.0)
...
Таким образом, 285K достигает производительности 14900K при **вдвое меньшем энергопотреблении**.
...
- **AI-нагрузки (NPU):** NPU на 13 TOPS ускоряет Windows Studio Effects и локальный AI-инференс — AMD аналога не имеет в настольном сегменте.
...
- **Gear 2 по умолчанию:** как и Alder/Raptor Lake, контроллер работает в Gear 2. Для частот выше 7200 может потребоваться Gear 4 — избегайте, latency резко растёт
...
- **Idle / веб-сёрфинг:** 30–40W (чиплетная архитектура чуть прожорливее монолита в простое)
...
Энергоэффективность Arrow Lake — **лучшая в истории Intel**. Флагман 125W — это то, чего ждали годами.
...
**Вывод:** 9950X выигрывает в чистом рендеринге и на 5–15% в играх. 285K берёт реванш в видеомонтаже (QuickSync) и AI-нагрузках (NPU). Выбор зависит от сценария.
...
- Пользователи, кому нужен NPU для Windows Studio Effects и локального AI
